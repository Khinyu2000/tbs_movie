...
$order = isset( $_GET["order"] ) ? preg_replace( "/[^_a-zA-Z]/", "", $_GET["order"] ) : "created_date";
 ...
 <section class="ssn-cat">
   ...
   <section class="ssn-cat-mani">
     <?php
      if($id):
        $items = Movies::getMoviesByMovieCategoryId($id, $order);
        $category = MovieCategories::getMovieCategoryById($id);
        if($category):
      ?>
       ...
       <div class="ssn-add-item-overlay">
         <form action="<?php echo URL ?>/movies/create_movie/" method="post" enctype="multipart/form-data">
           <input type="hidden" name="movie_category_id" value="<?php echo $category->getValue('id') ?>">
           <div>
           ...
           <div>
             ...
             <div class="ssn-input-container">
               <textarea name="full_video" rows="8" cols="80" placeholder="<iframe>...</iframe>" id="full_video"></textarea>
               <span class="ssn-line"></span>
             </div>
           </div>
           <div>
         ...
         <div class="hk-sub-cat-wrap">
           ...
           <?php if($items): ?>
             <div class="hk-order-link">
               <span>Order by: </span>
               <a <?php echo ($order=='name')? 'class="active-order"' : 'href="' . URL . '/movies/display/' . $id . '/' . '?order=name"' ?>>Name</a> |
               <a <?php echo ($order=='name')? 'href="' . URL . '/movies/display/' . $id . '/' . '?order=created_date"' : 'class="active-order"'?>>Created Date</a>
             </div>
             ...
             <ul class="hk-item-order-ajax">
               <?php foreach ($items as $item): ?>
                 <li class="marquee" data-id="<?php echo $item->getValue('id') ?>">
                   <a href="<?php echo URL . '/movies/detail/' . $item->getValue('id') ?>">
                     <img src="<?php echo URL . '/photos/movies/id_' . $item->getValue('id'). '_' . $item->getValue('photo') ?>" alt="<?php echo $item->getValue('name') ?>">
